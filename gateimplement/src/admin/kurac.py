...
class AdminPanel:
    ...
    def create_testing_tab(self):
        ...
        canvas = tk.Canvas(testing_frame, bg='#2b2b2b')
        scrollbar = ttk.Scrollbar(testing_frame, orient="vertical", command=canvas.yview)
        ...
        left_panel = ttk.LabelFrame(scrollable_frame, text="🔧 Test Configuration", padding=10)
        ...
        right_panel = ttk.LabelFrame(scrollable_frame, text="📊 Test Results", padding=10)
    ...
    def create_connection_management_tab(self):
        ...
        # Current IP and Port
        ttk.Label(info_grid, text="Current IP:", font=('Arial', 9, 'bold')).grid(row=0, column=0, sticky=tk.W,
                                                                                 padx=(0, 10))
    ...
    def save_verification_report(self, results: str, target_ip: str):
        ...
        self.log_activity("INFO", f"Port scan completed for {target_ip}")
...
def trace_route(self):
    ...
    try:
        ...
        result = subprocess.run(trace_cmd, shell=True, capture_output=True, text=True)
        self.log_activity("INFO", f"Traceroute executed for {target_ip}")

        # Show results in dialog
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Traceroute Results - {target_ip}")
        dialog.geometry("600x500")
        dialog.configure(bg='#2b2b2b')

        text_area = tk.Text(dialog, wrap=tk.WORD, bg='#1e1e1e', fg='#00ff00', font=('Consolas', 9))
        ...
        messagebox.showerror("Traceroute Error", f"Failed to trace route: {e}")
...
def refresh_connections(self):
    ...
    try:
        ...
        for room_id, room_info in self.active_rooms.items():
            self.rooms_tree.insert('', 'end', room_id,
                                   text=room_info['name'],
                                   values=(room_info['type'],
                                           len(room_info['clients']),
                                           room_info['status'],
                                           room_info['created']))

        # Repopulate clients
        for client_id, client_info in self.connected_clients.items():
            self.clients_tree.insert('', 'end', client_id,
                                     text=client_id,
                                     values=(client_info.get('ip', 'N/A'),
                                             client_info.get('room', 'N/A'),
                                             client_info.get('status', 'N/A'),
                                             client_info.get('connected', 'N/A')))

        self.log_activity("INFO", "Connections refreshed")

    except Exception as e:
        self.log_activity("ERROR", f"Connection refresh failed: {e}")
        messagebox.showerror("Refresh Error", f"Failed to refresh connections: {e}")
...
def broadcast_message(self):
    ...
    try:
        ...
        ttk.Label(dialog, text="Message to Broadcast:").pack(pady=(10, 5))

        message_text = tk.Text(dialog, height=8, wrap=tk.WORD,
                               bg='#3c3c3c', fg='#cccccc', font=('Consolas', 9))
        ...
        def send_broadcast():
            ...
            for client_id in self.connected_clients:
                self.log_activity("INFO", f"Broadcast to {client_id}: {message[:50]}...")

            messagebox.showinfo("Success", "Message broadcasted to all clients")
            dialog.destroy()

        button_frame = ttk.Frame(dialog)
        button_frame.pack(fill=tk.X, pady=10)

        ttk.Button(button_frame, text="Send", command=send_broadcast).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=dialog.destroy).pack(side=tk.LEFT, padx=5)

    except Exception as e:
        self.log_activity("ERROR", f"Broadcast failed: {e}")
        messagebox.showerror("Broadcast Error", f"Failed to broadcast message: {e}")
...
def export_connection_log(self):
    ...
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"connection_log_{timestamp}.json"
        ...
        self.log_activity("INFO", f"Connection log exported: {save_path}")
        messagebox.showinfo("Export Complete", f"Connection log exported to {save_path}")

    except Exception as e:
        self.log_activity("ERROR", f"Export log failed: {e}")
        messagebox.showerror("Export Error", f"Failed to export connection log: {e}")
            ...
            if self.is_connected:
                ...
                for client_id in list(self.connected_clients.keys()):
                    if random.random() < 0.1:  # 10% chance to update status
                        self.connected_clients[client_id]['status'] = random.choice(['Active', 'Idle'])
                        self.refresh_connections()

        except Exception as e:
            self.log_activity("ERROR", f"Connection monitoring error: {e}")
...
def run(self):
    ...
    try:
        ...
        messagebox.showerror("Application Error", f"Fatal error: {str(e)}")
